...
import DashboardTopbar from "@/components/dashboards/users/navbar/MobileNavbar";
...
export default function EmployeeLayoutClient({
  ...
  return (
    <div className="font-header flex min-h-screen bg-slate-50">
      {/* Desktop sidebar: part of the layout, so it stays mounted between pages */}
      <aside className="sticky top-0 hidden h-screen w-64 shrink-0 border-r border-slate-200 bg-white lg:block">
        <Sidebar dashboard="user" user={user} />
      </aside>

      <MobileSideBar dashboard="user" session={session} />

      <div className="flex min-w-0 flex-1 flex-col">
        <DashboardTopbar user={user} token={session?.user?.token} />
