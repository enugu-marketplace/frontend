...
interface SidebarProps {
  dashboard: string;
  /** Passed down from the server session in the layout, so nothing loads here */
  user?: SidebarUser;
}
  ...
  return (
    <div className="flex h-full w-full flex-col">
      <div className="border-b border-slate-200 px-4 py-4">
        <Link href="/admin-dashboard" className="inline-block" onClick={() => setShowSidebar(false)}>
          <Logo idSuffix="admin-sidebar" markClassName="h-9 w-9" showTagline={false} />
        </Link>
